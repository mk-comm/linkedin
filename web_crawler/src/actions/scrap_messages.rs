...
pub async fn scrap_message(conversation: &Conversation, page: &Page) -> Result<(), playwright::Error> {
    ...
    let message_container = page.query_selector("ul[class='msg-s-message-list-content list-style-none full-width mbA']").await.unwrap().unwrap(); // select the message container

    let owner_container = page.clone().query_selector("div[class='msg-title-bar global-title-container shared-title-bar']").await.unwrap().unwrap();
...
async fn create_message (message: &Message, full_name: &FullName, conversation: &Conversation) -> MessageCategory{
    ...
    let res = client.post("https://overview.tribe.xyz/api/1.1/wf/tribe_api_receive").json(&payload).send().await.unwrap();
...
async fn check_message(message: &Message, full_name: &FullName, conversation: &Conversation) -> ResponseApi {
    let client = reqwest::Client::new();
            let payload = json!({
                            "message_text": message.message_text,
                            "conversation_url": conversation.thread_url,
            ...
            let part_of_sequence = json_response["response"]["part_of_sequence"].as_bool().unwrap();
            ResponseApi { new_message, part_of_sequence }
}

async fn mark_unread(conversation_element: &ElementHandle) -> Result<(), playwright::Error> {
    
    let dropdown = conversation_element.query_selector("div[class='msg-conversation-card__inbox-shortcuts']").await?; // find 3 dots button
    ...
    let inner_container = conversation_element.query_selector("div[class=artdeco-dropdown__content-inner]").await?;
