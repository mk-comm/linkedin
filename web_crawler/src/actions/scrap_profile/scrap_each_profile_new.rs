...
pub async fn scrap_each_profile(
    ...
    //browser.switch_to_window(page.clone()).await?;
    ...
    let linkedin_url_new = linkedin_url.as_str().to_string().clone();
    let linkedin_url_update = Some(linkedin_url.clone());
    ...
    let experience_url = format!("{}details/experience", &linkedin_url_new);
    ...
    if !cookie {
        ...
        if !cookie_second_try {
            wait(1, 3);
            let screenshot = browser.screenshot_as_png().await?;
            let be = browser.clone().quit().await;
            send_screenshot(
                screenshot,
                "Scrap each profile",
                "Session cookie expired",
